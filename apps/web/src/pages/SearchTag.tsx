...
import {
  contentListByGenreQueryOptions,
  genresQueryOptions,
} from "@api/hooks/videoQueries";
...
const SearchTag = () => {
  ...
  const [activeGenreFilter, setActiveGenreFilter] = useState<Genre | null>(
    null,
  );
  ...
  const { data: genreData } = useSuspenseQuery(
    genresQueryOptions(type, StaticRequest.baseRequest),
  );
  ...
  useEffect(() => {
    ...
    const foundGenre = genreData.genres.find(
      (g) => g.id === Number(searchParams.get("ids")),
    );
    if (foundGenre) {
  ...
  return (
    <div className="sp-root">
      <div className="sp-genre-header">
        <div className="sp-genre-page-title">
          {type === "movie" ? "영화" : "TV"}
        </div>
      </div>
