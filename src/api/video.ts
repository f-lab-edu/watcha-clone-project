...
export const getTodayTrend = (req: SearchRequest): Promise<MovieListResponse> =>
  client
    .get<MovieListResponse>('/3/trending/all/day', { params: req })
    .then(resp => resp.data);

export const getGenresByType = (type: 'movie' | 'tv', req: SearchRequest): Promise<GenreListResponse> =>
  client
    .get<GenreListResponse>(`/3/genre/${type}/list`)
    .then(resp => resp.data);

export const getContentListByGenre = (type: 'movie' | 'tv', genreIds: number, req: SearchRequest): Promise<MovieListResponse> =>
  client
...
export const getTvDetail = (tvId: number, req: SearchRequest): Promise<MovieDetailResponse> =>
  client
    .get<MovieDetailResponse>(`/3/tv/${tvId}`, { params: { ...req, 'append_to_response': 'content_ratings,credits', } })
    .then(resp => resp.data);

export const getSimilarTvList = (tvId: number, req: SearchRequest): Promise<MovieListResponse> =>
  client
    .get<MovieListResponse>(`/3/tv/${tvId}/similar`, { params: req })
    .then(resp => resp.data);
